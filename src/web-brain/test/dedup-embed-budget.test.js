...
import { deduplicatePages, stripRepeatedNavigationLines, splitIntoBlocks, } from "../src/dedup.js";
import { hashEmbed, cosineSimilarity, VectorStore, indexCrawlResult, findSimilar, } from "../src/embed.js";
import { countTokens, applyAggregateMarkdownBudget, truncateByTokens, } from "../src/budget.js";
import { cookiesFromNetscape, cookiesFromStorageState, resolveAuth, } from "../src/auth.js";
...
describe("budget", () => {
    ...
    it("applyAggregateMarkdownBudget truncates later items", () => {
        ...
        applyAggregateMarkdownBudget(items, (i) => i.markdown, (i, b) => {
            i.markdown = b;
        ...
        expect(truncateByTokens("hello world. more text here.", 2).length).toBeLessThan(40);
    });
});
describe("auth", () => {
...
`;
    ...
    it("resolveAuth from cookie file", () => {
        ...
        writeFileSync(p, `.ex.com	TRUE	/	FALSE	0	tok	xyz\n`);
