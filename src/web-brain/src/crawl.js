...
import { isPathAllowed, isUrlAllowedByRobots, parseRobotsTxt, } from "./robots.js";
import { assertSafeHttpUrl, toWebError } from "./ssrf.js";
import { discoverSitemapUrls } from "./sitemap.js";
import { canonicalForCrawl, canonicalForOutput, matchesPatterns, prioritizeDocLinks, stripFragment, } from "./url-utils.js";
import { deduplicatePages } from "./dedup.js";
import { indexCrawlResult, isIndexingEnabled } from "./embed.js";
import { applyAggregateMarkdownBudget, buildEvidenceFromMarkdown, DEFAULT_MAX_TOTAL_CHARS, MIN_TOKENS_PER_PAGE, scaleDefaultTokens, truncateByChars, } from "./budget.js";
function sleep(ms) {
 ...
 */
export async function webCrawl(seedUrl, opts = {}) {
    ...
    const allowUrl = (url, opts) => {
        ...
        const isSeed = opts?.isSeed === true || canonicalForCrawl(url) === seedCanon;
        if (!isSeed &&
...
async function finalizeCrawl(args) {
    ...
    let droppedOverBudget = 0;
    {
        ...
        for (const page of pages) {
            if (charCount + page.markdown.length > args.maxTotalChars &&
                budgeted.length > 0) {
                droppedOverBudget += 1;
                continue;
            }
