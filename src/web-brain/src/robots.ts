...
export type RobotsRules = {
  /** Disallow path prefixes for User-agent: * (and matching agents). */
  disallows: string[];
  /** Allow path prefixes (override). */
  allows: string[];
...
export function parseRobotsTxt(text: string, userAgent = DEFAULT_UA): RobotsRules {
  ...
  const groups: { agents: string[]; disallows: string[]; allows: string[]; delay: number | null }[] =
    [];
  let current: {
    agents: string[];
    disallows: string[];
  ...
  for (const line of lines) {
    ...
    if (key === "user-agent") {
      ...
      else if (key === "allow") current.allows.push(value);
      else if (key === "crawl-delay") {
        const n = Number(value);
        if (Number.isFinite(n) && n >= 0) current.delay = Math.round(n * 1000);
      }
    }
  }
  flush();
  ...
  const match =
    groups.find((g) => g.agents.some((a) => a !== "*" && (ua === a || ua.startsWith(a)))) ||
...
export function isPathAllowed(pathname: string, rules: RobotsRules): boolean {
  ...
  for (const d of rules.disallows) {
    if (d === "") continue; // empty disallow means allow all for that agent in some crawlers
