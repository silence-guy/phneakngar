...
export function ddgTimeParam(timeRange?: SearchTimeRange): string | undefined {
 ...
 */
export function resolveDdgResultHref(href: string): string | null {
  let raw = href.trim();
  if (!raw) return null;
  ...
  if (/^https?:\/\//i.test(raw) && !raw.includes("duckduckgo.com")) return raw;
  return null;
}

function detectBlockedHint(html: string): string | undefined {
...
export function parseDdgLiteHtml(html: string, maxResults: number): WebSearchResult[] {
  ...
  const push = (url: string | null, titleRaw: string, snippet = "") => {
  ...
  while ((m = anchorRe.exec(html)) !== null && results.length < maxResults) {
    ...
    const href = hrefM[1];
    const isResultClass =
      /class\s*=\s*["'][^"']*result(?:__a|-link)/i.test(attrs) ||
      /rel\s*=\s*["'][^"']*nofollow/i.test(attrs) ||
      /uddg=/i.test(href);
    if (!isResultClass) continue;
...
async function runHtmlProvider(
  name: string,
  url: URL,
  maxResults: number,
...
function applyDdgParams(u: URL, query: string, timeRange?: SearchTimeRange): void {
  u.searchParams.set("q", query);
  const df = ddgTimeParam(timeRange);
  if (df) u.searchParams.set("df", df);
}
...
export const ddgHtmlProvider: SearchProvider = {
  name: "ddg-html",
  async search(query, { maxResults, fetchImpl, timeRange }) {
...
export async function webSearch(
  ...
  const chain: SearchProvider[] = opts.noFallback
  ...
  const telemetry: SearchProviderTelemetry[] = [];
  const tried: string[] = [];
