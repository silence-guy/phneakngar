...
export function truncateByTokens(text: string, maxTokens: number): string {
  ...
  if (lastPara > threshold) return head.slice(0, lastPara) + TRUNC_MARKER;
...
export function applyAggregateMarkdownBudget<T>(
  ...
  opts: { maxTokensOut: number; minTokensPerItem?: number },
): void {
  let used = 0;
...
export function buildEvidenceFromMarkdown(
  query: string,
  title: string,
  url: string,
  markdown: string,
  opts: { maxTokensOut?: number; maxItems?: number } = {},
): EvidenceItem[] {
  if (!markdown) return [];
...
export function scaleDefaultTokens(pageCount: number): number {
  return Math.min(
    MAX_TOKENS_OUT_CEILING,
    Math.max(DEFAULT_MAX_TOKENS_OUT, PER_PAGE_TOKENS * Math.max(1, pageCount)),
  );
}
