...
export type DedupPage = { url: string; markdown: string };

export function splitIntoBlocks(markdown: string): string[] {
  if (!markdown.trim()) return [];
  const lines = markdown.split("\n");
  const headingIndices: { lineIdx: number }[] = [];
...
export function stripRepeatedNavigationLines(pages: DedupPage[]): DedupPage[] {
  ...
  for (const lines of lineSets) {
    ...
    for (
      ...
      i >= Math.max(lines.length - MAX_TRAILING_LINES, 0);
      i--
    ) {
      const h = lineHash(lines[i]!);
      if (!seenT.has(h)) {
  ...
  const navLeading = new Set(
    [...countLeading].filter(([, c]) => c >= threshold).map(([h]) => h),
  );
  const navTrailing = new Set(
    [...countTrailing].filter(([, c]) => c >= threshold).map(([h]) => h),
  );
...
function defaultBoilerplatePath(): string {
  return join(
...
export function getStoredBoilerplate(
  domain: string,
  storeDir = defaultBoilerplatePath(),
): string[] {
  const p = join(storeDir, `${domain.replace(/[^a-z0-9.-]/gi, "_")}.json`);
  if (!existsSync(p)) return [];
  try {
    const raw = JSON.parse(readFileSync(p, "utf-8")) as { hashes?: string[] };
...
export function storeBoilerplate(
  domain: string,
  hashes: string[],
  storeDir = defaultBoilerplatePath(),
): void {
  ...
  writeFileSync(
    p,
    JSON.stringify({ domain, hashes, updatedAt: new Date().toISOString() }),
    { mode: 0o600 },
  );
}
...
export function deduplicatePages(
  ...
  const boilerplateHashes = new Set<string>(
    domain ? getStoredBoilerplate(domain, dir) : [],
  );
  ...
  for (const [hash, count] of hashPageCount) {
    if (count > threshold) boilerplateHashes.add(hash);
  }
  ...
  return pageBlocks.map((page) => {
    const filtered = page.blocks.filter(
      (block) => !boilerplateHashes.has(hashBlock(block)),
    );
    return {
      url: page.url,
