...
import { createDb, queries, createLogger, validateWsConnectionTicket, WS_CHHLAT_TICKET_AUDIENCE, WS_TICKET_TTL_SECONDS, WS_USER_TICKET_AUDIENCE, } from "@phneakngar/shared";
...
function chhlatDoName(workspaceId, chhlatId) {
...
export class WebSocketDurableObject extends DurableObject {
    ...
    async webSocketMessage(ws, message) {
        ...
        if (msg.type === "auth") {
            ...
            if (msg.machineToken && msgChhlatId) {
                ...
                ws.serializeAttachment({ type: "chhlat", workspaceId: state.workspaceId, chhlatId: msgChhlatId, userId: authResult.userId, authenticated: true });
    ...
    async notifyUserDO(userId, payload) {
    ...
    async getChhlatIdForUser(userId, workspaceId) {
    ...
    async consumeTicketNonce(payload) {
    ...
    async cleanupExpiredTicketNonces(nowSeconds) {
    ...
    async validateMachineToken(token, workspaceId, chhlatId) {
        ...
        if (!mt)
            return null;
        if (mt.status !== "active" || mt.workspaceId !== workspaceId)
            return null;
        if (mt.hostname !== chhlatId)
            return null;
        const machine = await queries.machine.getMachineByChhlat(db, chhlatId, mt.workspaceId);
        if (!machine || machine.ownerId !== mt.userId)
            return null;
