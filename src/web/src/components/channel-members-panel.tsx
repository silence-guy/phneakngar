...
function toRow(item: ChannelMemberItem): ChannelMemberRow {
  return {
    id: item.id,
...
/**
 * Compact channel participants control (C8).
 * Progressive disclosure: icon + count → popover list with add/remove agents.
 * Wired against existing `/api/channels/[id]/members` (workspace-scoped).
 */
export function ChannelMembersPanel({
  /** Prefer highlighting / auto-suggest this agent when adding. */
  preferAgentId,
  className,
}: {
  ...
  const { channels, activeChannel, loading: channelsLoading } = useChannel();
  const { agents } = useAgentContext();
  const labels = AGENT_CHAT_LABELS.participants;
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [listRes, wsMembers] = await Promise.all([
        listChannelMembers(channel.id, workspaceId),
  ...
  }, [channel?.id, workspaceId, labels.failedToLoad]);
  ...
  // Refresh badge count when channel changes even if closed (lightweight).
  useEffect(() => {
    if (!channel?.id || !workspaceId) {
      setMembers([]);
      return;
  ...
  const resolved = useMemo(
    () =>
      resolveChannelMembers(members.map(toRow), {
        agents: agentNameMap,
        users: userNameMap.names,
        userEmails: userNameMap.emails,
      }),
    [members, agentNameMap, userNameMap],
  );
  ...
  const availableAgents = useMemo(
    ...
    [agents, members, preferAgentId],
  );

  const agentCount = countAgentMembers(members.map(toRow));

  const handleAddAgent = async (agentId: string) => {
    if (!channel?.id || busyKey) return;
    setBusyKey(`add:${agentId}`);
    try {
      const res = await addChannelMember(channel.id, workspaceId, {
        member_type: "agent",
        member_id: agentId,
      });
      setMembers((prev) => {
  ...
  const handleRemove = async (memberType: "user" | "agent", memberId: string) => {
    ...
    try {
      await removeChannelMember(channel.id, workspaceId, memberType, memberId);
      setMembers((prev) =>
        prev.filter(
