...
function post(body: unknown, headers?: Record<string, string>) {
  return POST(
    new NextRequest("http://localhost/api/webhooks/lark", {
      method: "POST",
      headers: {
...
describe("POST /api/webhooks/lark", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 503 when map is set without secret", async () => {
    ...
    const res = await post({ tenant_key: "tenant_1", text: "hello" });
  ...
  it("returns 401 when secret mismatches", async () => {
    const res = await post(
      { tenant_key: "tenant_1", text: "hello" },
  ...
  it("forwards provider lark to ingress", async () => {
    ...
    const res = await post({
      tenant_key: "tenant_1",
      event: { message: { chat_id: "oc_1", content: JSON.stringify({ text: "hi" }) } },
    });
    expect(res.status).toBe(200);
    expect(mockIngress).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ provider: "lark" }),
    );
  });
});
