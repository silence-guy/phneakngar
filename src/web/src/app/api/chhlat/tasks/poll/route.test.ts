...
const mockGetWorkspaceAgentLanguageMode = vi.fn().mockResolvedValue(null);
...
vi.mock("@phneakngar/shared", async (importOriginal) => {
  ...
  queries: {
    runtime: {
      getRuntimeIdsByChhlat: (...args: unknown[]) => mockGetRuntimeIdsByChhlat(...args),
    },
    machine: {
...
vi.mock("@/lib/cache", () => ({
  ...
  cacheKeys: {
    ...
    runtimeIds: (wsId: string, chhlatId: string) => `rt:${wsId}:${chhlatId}`,
    agent: (wsId: string, agentId: string) => `ag:${wsId}:${agentId}`,
    heartbeat: (wsId: string, chhlatId: string) => `hb:${wsId}:${chhlatId}`,
...
function postReq(body: unknown) {
  return new NextRequest("http://localhost/api/chhlat/tasks/poll", {
    method: "POST",
...
describe("POST /api/chhlat/tasks/poll", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("withholds tasks and requests an update below MIN_CLI_VERSION", async () => {
    ...
    const res = await POST(postReq({ chhlat_id: "d1", cli_version: "1.1.9" }));
  ...
  it("returns evicted: true when chhlat has no runtimes", async () => {
  ...
  it("resolves runtime IDs from chhlat_id", async () => {
    mockGetRuntimeIdsByChhlat.mockResolvedValue(["r1", "r2"]);
    mockSweepStaleState.mockResolvedValue(undefined);
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
    ...
    expect(mockGetRuntimeIdsByChhlat).toHaveBeenCalledWith({}, "d1", "w1");
  });
  ...
  it("returns tasks with agent data for claimed tasks", async () => {
    ...
    const res = await POST(postReq({ chhlat_id: "d1", max_tasks: 3 }));
    const body = await res.json();
  ...
  it("does not broadcast runtime.status (heartbeat handles that)", async () => {
    mockGetRuntimeIdsByChhlat.mockResolvedValue(["r1", "r2", "r3"]);
    mockSweepStaleState.mockResolvedValue(undefined);
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
  ...
  it("returns pending_update when pendingUpdateVersion is set and cli_version is older, without clearing flag", async () => {
    ...
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
    mockGetMachineByChhlat.mockResolvedValue({ pendingUpdateVersion: "1.0.0" });

    const res = await POST(postReq({ chhlat_id: "d1", cli_version: "0.5.0" }));
    const body = await res.json();
  ...
  it("does not return pending_update when pendingUpdateVersion is not set", async () => {
    ...
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
    mockGetMachineByChhlat.mockResolvedValue({ pendingUpdateVersion: null });
  ...
  it("auto-clears pendingUpdateVersion and broadcasts when cli_version >= pending version", async () => {
    ...
    const res = await POST(postReq({ chhlat_id: "d1", cli_version: "1.0.0" }));
    const body = await res.json();
  ...
  it("still returns tasks when pending check fails (D1 transient error)", async () => {
    ...
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
    mockGetMachineByChhlat
      .mockResolvedValueOnce(null)
      .mockRejectedValueOnce(new Error("D1 timeout"));

    const res = await POST(postReq({ chhlat_id: "d1", cli_version: "0.1.0" }));
    const body = await res.json();
  ...
  it("poll does not write KV heartbeat (moved to /api/chhlat/heartbeat)", async () => {
    mockGetRuntimeIdsByChhlat.mockResolvedValue(["r1"]);
    mockSweepStaleState.mockResolvedValue(undefined);
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
  ...
  it("broadcasts runtime.status after clearing pendingRescan", async () => {
    ...
    mockBroadcastToUser.mockResolvedValue(undefined);
    mockClaimTasksForRuntimes.mockResolvedValue([]);
    mockGetMachineByChhlat.mockResolvedValue({ pendingRescan: true });
