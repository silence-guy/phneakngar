...
describe("GET /api/chhlat/tasks/[taskId]/status", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns task status", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/chhlat/tasks/t1/status"),
      withParams("t1")
    );
  ...
  it("returns 404 when task is not found for the token workspace", async () => {
    ...
    const res = await GET(
      new NextRequest("http://localhost/api/chhlat/tasks/t-missing/status"),
