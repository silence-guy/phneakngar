...
// session/JWT does NOT. This lets us exercise the route's 403 chhlat-only guard.
...
function makeReq(body: unknown, opts?: { machine?: boolean }) {
  return new NextRequest("http://localhost/api/chhlat/conversations/c1/messages", {
    method: "POST",
    headers: {
...
describe("POST /api/chhlat/conversations/[id]/messages", () => {
  // TC1
  ...
  // TC5 — a user JWT (no workspaceId) → 403 chhlat-only
