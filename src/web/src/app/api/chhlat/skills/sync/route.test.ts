...
describe("POST /api/chhlat/skills/sync", () => {
  it("403 when no workspace (session token, not a machine token)", async () => {
    ...
    const res = await post({ scope: "global", runtime: "claude", skills: [], chhlat_id: "d1" });
  ...
  it("syncs agent skills when scope=agent", async () => {
    const res = await post({ scope: "agent", agent_id: "a1", runtime: "claude", skills: [], chhlat_id: "d1" });
    expect(res.status).toBe(200);
    expect(mockSyncAgent).toHaveBeenCalledWith({}, "a1", "claude", "w1", []);
  });

  it("400 when scope=agent without agent_id", async () => {
    const res = await post({ scope: "agent", runtime: "claude", skills: [], chhlat_id: "d1" });
