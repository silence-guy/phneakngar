...
export function parseSkillPack(raw: unknown): SkillPack {
  if (!isRecord(raw)) throw new Error("skill pack must be an object");
  const skillsRaw = raw.skills;
  if (!Array.isArray(skillsRaw)) throw new Error("skill pack.skills must be an array");

  const skills: SkillPackItem[] = [];
  for (const item of skillsRaw) {
    if (!isRecord(item)) throw new Error("skill pack entry must be an object");
    const name = typeof item.name === "string" ? item.name.trim() : "";
    if (!name) throw new Error("skill pack entry requires name");
...
export function mergeSkillsByName(
  existing: SkillPackItem[],
  incoming: SkillPackItem[],
): SkillPackItem[] {
  const byName = new Map<string, SkillPackItem>();
...
export function toSkillPack(
  skills: SkillPackItem[],
  opts?: { source?: string; exportedAt?: string },
): SkillPack {
  return {
    version: 1,
    skills,
...
export function skillPackToJson(pack: SkillPack): string {
