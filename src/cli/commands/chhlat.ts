...
async function startInBackground(
  ...
  console.error(
    `Chhlat did not write a pidfile within ${PID_POLL_TIMEOUT_MS}ms — check logs: ${logPath}`,
  );
  process.exit(1);
}
...
function statusCommand(profile: string | undefined): void {
  ...
  if (!isProcessAlive(pid)) {
    console.log(
      `Chhlat not running (stale pidfile at ${pidFilePath(profile)}).${profileSuffix}`,
    );
    return;
  }
...
async function stopCommand(profile: string | undefined): Promise<void> {
  ...
  console.warn(
    `Chhlat did not exit within ${shutdownMs}ms — force killing.`,
  );
  if (!isWindows) {
    try {
