...
export function calendarCommand() {
    const cmd = new Command("calendar").description("Manage scheduled agent events");
    cmd
        .command("set")
        .description("Create a calendar event")
        .option("--agent_id <id>", "Agent ID")
        .requiredOption("--event_title <title>", "Event title (used as the task prompt)")
        .requiredOption("--datetime <iso>", "Scheduled datetime (YYYY-MM-DDTHH:MM, local time)")
        ...
        .option("--repeat_stop_date <date>", "Stop repeating on or after this date (YYYY-MM-DD, local time)")
        ...
        if (process.env.PHNEAKNGAR_CONVERSATION_ID)
            body.conversation_id = process.env.PHNEAKNGAR_CONVERSATION_ID;
        try {
    ...
    cmd
        ...
        const client = new APIClient(serverUrl, token, workspaceId);
        const now = Date.now();
        const from = new Date(now - Number(opts.past_days) * 86_400_000).toISOString();
        const to = new Date(now + Number(opts.future_days) * 86_400_000).toISOString();
        try {
    ...
    cmd
        ...
        .option("--datetime <iso>", "New scheduled datetime (YYYY-MM-DDTHH:MM, local time)")
        .option("--repeat <interval>", "New repeat interval, e.g. 1day, 2hour")
        .option("--clear_repeat", "Convert a repeating event into a one-off")
        .option("--repeat_stop_date <date>", "New stop date (YYYY-MM-DD, local time)")
