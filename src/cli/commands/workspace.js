...
async function resolveWorkspaceId(client, configName) {
    ...
    try {
        const newWs = await client.postJSON("/api/workspaces", { name: wsName, slug: slugify(wsName) });
    ...
    cmd
        ...
        if (!targetWorkspaceId) {
            ...
            for (let attempt = 0; attempt < 15; attempt++) {
                try {
                    runtimes = await wsClient.getJSON("/api/runtimes");
            ...
            try {
                const res = await wsClient.postJSON("/api/studios", payload);
        ...
        try {
            ...
            if (agents.length > 0) {
                ...
                const newWs = await targetClient.postJSON("/api/workspaces", { name: wsName, slug: slugify(wsName) });
