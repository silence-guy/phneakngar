...
function normalizeSendResponse(res) {
    if (res &&
        typeof res === "object" &&
        "status" in res &&
    ...
    return { email: res, pendingApproval: false };
}
function uniqueAttachmentFilename(rawFilename, index, usedFilenames) {
...
export function emailCommand() {
    ...
    cmd
        ...
        try {
            ...
            if (opts.email_id) {
                ...
                emails = await client.getJSON(query);
            }
            if (!emails.length) {
            ...
            console.log(`Downloaded ${emails.length} email${emails.length === 1 ? "" : "s"} to ${emailDir_base}/`);
    ...
    cmd
        ...
        .option("--attachment <path>", "Path to a file to attach (repeatable)", collectRepeated, [])
        .option("--requires-approval", "Queue the send for human approval instead of delivering immediately")
        .option("--workspace <id>", "Workspace ID")
        .action(async (opts, command) => {
        const agentId = resolveAgentId(opts);
        ...
        try {
            ...
            if (normalized.pendingApproval) {
                const approvalSuffix = normalized.approvalId
                    ...
                    : "";
                console.log(`Queued email to ${normalized.email.to_email} for approval (id: ${normalized.email.id})${approvalSuffix}`);
            }
            else {
    ...
    cmd
        ...
        .option("--attachment <path>", "Extra file to attach (repeatable)", collectRepeated, [])
        .option("--requires-approval", "Queue the forward for human approval instead of delivering immediately")
        .option("--workspace <id>", "Workspace ID")
        .action(async (opts, command) => {
        const agentId = resolveAgentId(opts);
        const { serverUrl, token, workspaceId } = resolveClientOpts(command, {
        ...
        try {
            ...
            if (normalized.pendingApproval) {
                const approvalSuffix = normalized.approvalId
                    ? ` (approval: ${normalized.approvalId})`
                    : "";
                console.log(`Queued forward to ${normalized.email.to_email} for approval (id: ${normalized.email.id})${approvalSuffix}`);
            }
            else {
                console.log(`Forwarded email to ${normalized.email.to_email} (id: ${normalized.email.id})`);
            }
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    const whitelistCmd = new Command("whitelist").description("Manage email whitelist (allowed senders)");
    whitelistCmd
        .command("list")
    ...
    whitelistCmd
        ...
        const client = new APIClient(serverUrl, token, workspaceId);
        try {
            const entry = await client.postJSON(`/api/agents/${agentId}/whitelist`, { email: email.toLowerCase() });
