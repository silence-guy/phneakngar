...
import { configDir, configPath, loadCLIConfig, saveCLIConfig, } from "../lib/config.js";
...
export function initCommand() {
    return new Command("init")
        ...
        .option("--server <url>", "Control plane base URL (default: https://phneakngar-web.thatsilenceguy.workers.dev)")
        ...
        const parentServer = command.parent?.opts()?.server;
        const server = opts.server || parentServer;
        try {
            ...
            console.log(`  Status:  ${result.created ? "created" : result.updated ? "updated" : "unchanged"}`);
