...
export function syncCommand() {
    ...
    cmd
        ...
        const client = new APIClient(serverUrl, token, workspaceId);
        try {
            const result = await client.postJSON(`/api/chhlat/conversations/${encodeURIComponent(conversationId)}/messages`, { content, task_id: process.env.PHNEAKNGAR_TASK_ID || undefined });
            printJSON(result);
        }
        catch (err) {
