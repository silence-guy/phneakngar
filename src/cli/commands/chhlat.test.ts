...
vi.mock("../chhlat/chhlat.js", () => ({
  startChhlat: vi.fn(async () => {}),
}));

vi.mock("../chhlat/pidfile.js", () => ({
  readChhlatPid: vi.fn(() => null),
  isProcessAlive: vi.fn(() => false),
  removePidFileIfMatches: vi.fn(),
}));

vi.mock("../chhlat/config.js", () => ({
  chhlatLogFilePath: vi.fn(() => "/tmp/phneakngar/chhlat/logs/2026-04-17.log"),
...
} from "../chhlat/pidfile.js";
import { chhlatCommand } from "./chhlat.js";

const startChhlatMock = startChhlat as unknown as ReturnType<typeof vi.fn>;
const readChhlatPidMock = readChhlatPid as unknown as ReturnType<typeof vi.fn>;
...
function resetAll() {
  startChhlatMock.mockClear();
  readChhlatPidMock.mockClear();
  readChhlatPidMock.mockReturnValue(null);
...
// Mount the chhlat subcommand under a root program with the same top-level
...
async function runCLI(
  ...
  try {
    ...
    await program.parseAsync([...rootFlags, "chhlat", ...args], {
      from: "user",
    });
  } catch (e) {
...
describe("chhlat stop", () => {
  ...
  it("sends SIGTERM and reports clean stop when process exits", async () => {
    readChhlatPidMock.mockReturnValue(777);
    let alive = true;
  ...
  it("cleans up stale pidfile and reports not running when pid is dead", async () => {
    readChhlatPidMock.mockReturnValue(444);
    isProcessAliveMock.mockReturnValue(false);
    killSpy = vi.spyOn(process, "kill").mockImplementation(() => true);
...
describe("chhlat start (foreground)", () => {
  beforeEach(resetAll);

  it("delegates to startChhlat and does not spawn", async () => {
    await runCLI(["start", "--foreground"]);
    expect(startChhlatMock).toHaveBeenCalledTimes(1);
    expect(spawnMock).not.toHaveBeenCalled();
  });
});

describe("chhlat start (background)", () => {
  beforeEach(() => {
    resetAll();
    vi.useFakeTimers();
  ...
  it("spawns detached with log fds and reconstructed args", async () => {
    // Initial pre-check: no chhlat running. After spawn: pidfile has pid 55555.
    let calls = 0;
    readChhlatPidMock.mockImplementation(() => (++calls === 1 ? null : 55555));
    ...
    expect(startChhlatMock).not.toHaveBeenCalled();
    ...
    expect(out.join("\n")).toContain("Chhlat started (pid=55555)");
    expect(out.join("\n")).toContain("Logs: /tmp/phneakngar/chhlat/logs/2026-04-17.log");
  });

  it("forwards --server to child argv", async () => {
    let calls = 0;
    readChhlatPidMock.mockImplementation(() => (++calls === 1 ? null : 101));
  ...
  it("forwards root-level --profile to child argv", async () => {
    let calls = 0;
    readChhlatPidMock.mockImplementation(() => (++calls === 1 ? null : 202));
  ...
  it("forwards root-level --server to child argv", async () => {
    let calls = 0;
    readChhlatPidMock.mockImplementation(() => (++calls === 1 ? null : 303));
  ...
  it("warns and exits non-zero when pidfile never appears", async () => {
    readChhlatPidMock.mockReturnValue(null); // never set
    isProcessAliveMock.mockReturnValue(false);
    ...
    expect(err.join("\n")).toContain("/tmp/phneakngar/chhlat/logs/2026-04-17.log");
  });

  it("refuses to start when another chhlat is already alive", async () => {
    // Pre-existing alive chhlat: readChhlatPid returns pid, isProcessAlive true
    readChhlatPidMock.mockReturnValue(333);
    isProcessAliveMock.mockReturnValue(true);

    const { err } = await runCLI(["start"]);

    expect(spawnMock).not.toHaveBeenCalled();
    expect(err.join("\n")).toContain("Chhlat already running (pid=333)");
  });
