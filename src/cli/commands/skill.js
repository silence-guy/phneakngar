...
import { mergeSkillsByName, parseSkillPack, skillPackToJson, toSkillPack, } from "../lib/skill-pack.js";
...
export function skillCommand() {
    ...
    cmd
        ...
        if (agentId) {
            try {
                ...
                const res = await client.getJSON(`/api/agents/${encodeURIComponent(agentId)}/skills`);
                ...
                if (process.env.PHNEAKNGAR_DEBUG) {
                    console.error(`skill export API failed, falling back to dry-run: ${err instanceof Error ? err.message : err}`);
                }
            }
        }
        if (source === "empty") {
    ...
    cmd
        ...
        if (opts.dryRun) {
            if (opts.json)
                return printJSON({ dry_run: true, skills: pack.skills });
        ...
        console.log(`Imported ${incoming.skills.length} skill(s) (total ${merged.length}) → ${localPath}` +
