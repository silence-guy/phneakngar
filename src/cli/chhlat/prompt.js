import { buildAgentPromptLanguagePolicy, buildJudgmentPolicyNotice, formatMemoryForPrompt, readJudgmentPolicy, resolveAmbiguousDmJudgment, } from "@phneakngar/shared";
import { localISOString } from "./execenv/timeline.js";
const DM_RESPONSE_NOTICE = "Reply with `phneakngar sync send-dm` — that's the only thing the user sees; your task output and reasoning are not shown." +
...
const EMAIL_NOTICE = "This task was triggered by an incoming email. Reply to the sender via email — use the email sending tool to respond." +
...
const CALENDAR_NOTICE = "This task was triggered by a scheduled calendar event." +
    " If you need to communicate with someone, send an email using the email sending tool." +
    " If you need more information or confirmation, email them and then exit." +
    " Do not wait — when they reply, a new task will be triggered automatically and you will be woken up with their response.";
const WEB_BRAIN_NOTICE = " Live web tools are available via the lean web brain:" +
...
const ISSUE_NOTICE = "This task was triggered by an assigned issue. The issue_id is provided in this message." +
...
function mergedLanguagePolicyFor(tasks) {
...
function buildEmailDmNotice(name, email) {
    return (`This task was triggered by an incoming email on a conversation with ${name} (${email}).` +
...
export function buildTaskObject(task, attachments) {
...
export function buildPrompt(task, attachments) {
    return JSON.stringify(buildTaskObject(task, attachments));
}
export function buildMergedPrompt(tasks, attachmentsMap) {
