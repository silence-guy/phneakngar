...
import { loadChhlatConfig, sessionRunnerLogDir, chhlatLogFilePath } from "./config.js";
import { createHealthServer, detectHeadroomHealth } from "./health.js";
import { detectVersion, createBackend } from "./agent/index.js";
import { fromApiTask } from "./types.js";
import { writeMarkerFile, downloadAttachments } from "./session-runner.js";
...
const log = createLogger({ module: "chhlat" });
import { cmdPrefix } from "../lib/env.js";
import { acquireChhlatPid, releaseChhlatPid } from "./pidfile.js";
import { handleCliUpdate, isUpdating, readUpdateMarker, clearUpdateMarker } from "./update-handler.js";
import { findRunningPidByTaskId, findSupersedablePredecessor, steerWarmupGraceMs, updateEntry } from "./execenv/timeline.js";
import { isAlive, killGraceMs } from "./kill-tree.js";
import { writeKillIntent, readKillIntent, clearKillIntent, acquireSteeringLock, releaseSteeringLock, cleanupStaleIntents, } from "./execenv/steering.js";
import { ensureMailboxDirs, writeSteerMessage, waitForAck, inboxDir as steeringInboxDir, } from "./steering/mailbox.js";
...
function headroomHealthForRegister(health) {
    return {
...
export function pruneSessionRunnerLogs() {
...
export async function reconcilePendingCompletions(workspacesRoot) {
    ...
    for (const name of jsonFiles) {
        ...
        try {
            ...
            const client = new ChhlatClient(marker.serverURL);
            try {
...
export async function startChhlat(profile, serverUrl) {
    pruneSessionRunnerLogs();
    if (!acquireChhlatPid(profile)) {
        process.exit(1);
    }
    ...
    // pidfile. `releaseChhlatPid` only deletes when the file still points at us,
    // so a newer chhlat's pidfile is safe.
    process.once("exit", () => releaseChhlatPid(profile));
    const bailOnUnexpected = (label, err) => {
        log.error(`${label} — shutting down`, err);
        releaseChhlatPid(profile);
        process.exit(1);
    };
    process.once("uncaughtException", (err) => bailOnUnexpected("uncaughtException", err));
    process.once("unhandledRejection", (err) => bailOnUnexpected("unhandledRejection", err));
    const config = loadChhlatConfig(profile);
    if (serverUrl)
    ...
    if (workspaces.length === 0) {
        log.info("No workspaces configured — chhlat starting in standby mode. Register a workspace to begin.");
    }
    ...
    if (cliConfig.server_url)
        config.serverURL = cliConfig.server_url;
    const client = new ChhlatClient(config.serverURL);
    ...
    log.info(`Chhlat started — ${allRuntimeIds.length} runtime(s) across ${workspaceStates.length} workspace(s)`);
    ...
    const pollCycle = async () => {
        ...
        const results = await Promise.all(workspaceStates.map((ws) => client
            .poll(ws.token, config.chhlatId, budgetPerWs, config.cliVersion)
            .then((r) => ({ ws, r }))
            .catch((e) => ({ ws, e }))));
        ...
        for (const result of results) {
            ...
            if (pending_rescan) {
                log.info('Rescan requested — restarting chhlat to re-detect runtimes');
    ...
    const heartbeatPing = () => {
        for (const ws of workspaceStates) {
            client.heartbeat(ws.token, config.chhlatId).catch((e) => {
    ...
    // Any active machine token suffices — WS auth validates chhlat-level access,
    ...
    function handleWsPush(msg) {
        ...
        switch (type) {
            ...
            case "chhlat.update":
                ...
                break;
            case "chhlat.rescan":
                log.info("WS rescan requested — restarting chhlat");
                requestRestart();
                break;
            case "chhlat.kill": {
                const ws = wsMap.get(msg.workspaceId);
    ...
    const sweepTick = async () => {
        for (const ws of workspaceStates) {
            client.sweep(ws.token, config.chhlatId).catch((e) => {
    ...
    const shutdown = async () => {
        ...
        try {
            for (const ws of workspaceStates) {
                await client.deregister(ws.token, config.chhlatId);
            }
        }
        catch {
        ...
        health.server.close(() => {
            if (restartRequested) {
                ...
                const args = [entry, "chhlat", "start", "--foreground"];
                if (profile)
                ...
                try {
                    ...
                    log.error(`Failed to open chhlat log file ${logPath}`, e);
                }
                ...
                if (logFd != null)
                    closeSync(logFd);
                log.info(`Spawned new chhlat (pid=${child.pid}), logs: ${logPath}`);
            }
    ...
    process.on("SIGHUP", async () => {
        ...
        try {
            ...
            const freshWorkspaces = (freshConfig.watched_workspaces || [])
                .filter((ws) => ws.status !== "deleted" && !!ws.id);
...
export function spawnSessionRunner(input) {
    const logDir = sessionRunnerLogDir();
...
async function handleFileRequest(client, config, workspaceId, req, token) {
    ...
    try {
        if (req.request_type === "tree") {
            const entries = await readDirectoryTree(resolved, agentWorkdir, join(agentWorkdir, req.path));
...
async function handleTask(client, config, runtimeIndex, task, token, activeTasks, pendingSteer) {
            ...
            try {
                ...
                if (result) {
                    ...
                    if (!existing) {
                        ...
                        let ownerWake;
                        const ownerSignal = new Promise((resolve) => { ownerWake = resolve; });
                        const entry = {
                            tasks: [task],
                        ...
                        if (!predecessor) {
                            log.info(`Steering: predecessor ${result.pending.task_id} warming up; ${task.id} waiting`);
                            const POLL_MS = 200;
                            ...
                            while (Date.now() - waitStart < MAX_WAIT_MS) {
                                ...
                                if (!r) {
                                    log.info(`Steering: predecessor vanished; ${task.id} proceeding`);
                                    break;
                                }
                                if ("entry" in r) {
                                    predecessor = r.entry;
                            ...
                            if (config.enableSteering && isPersistent && predecessor.pid != null) {
                                ...
                                try {
                                    ...
                                    if (ackResult.acked) {
                                        ...
                                        catch { /* best effort — already started above */ }
