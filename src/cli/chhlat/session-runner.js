 ...
 *
 * Spawned by the chhlat as a detached child (`spawn('bun', ['run', thisFile, base64Input])`).
 * Survives chhlat restarts: owns the full lifecycle of message syncing, timeline writes,
 * log capture, and server completion reporting.
 */
import { mkdir, writeFile, rm, rename } from "fs/promises";
import { mkdirSync } from "fs";
import path from "path";
import { ChhlatClient, ChhlatHttpError, isTaskAlreadyTerminalError, makeClientToolActionApprovalCreator, } from "./client.js";
import { setToolActionApprovalCreator } from "./tool-gate.js";
import { createBackend } from "./agent/index.js";
import { setApprovalHoldPoller, setApprovalHoldRuntimeConfig, } from "./agent/claude.js";
import { prepare } from "./execenv/index.js";
import { initEntryAsync, updateEntry, createTimelineEntry, findResumableSessionByContextKey, } from "./execenv/timeline.js";
import { readKillIntent, clearKillIntent } from "./execenv/steering.js";
import { watchInbox, writeAck, writeNack, cleanupInboxFile, cleanupSteeringDir } from "./steering/mailbox.js";
import { RuntimeTurnState } from "./steering/turnState.js";
import { createInitialApmState, reduceApmGatedToolUse, reduceApmGatedCompaction, reduceApmGatedEnqueue, reduceApmGatedTurnEnd, reduceApmGatedError, reduceApmGatedFlushReadiness, reduceApmGatedRecentEvent, reduceApmStalledRecoveryTermination, reduceApmStartupTimeoutTermination, } from "./steering/apmStateMachine.js";
...
export async function writeMarkerFile(workspacesRoot, marker) {
...
export async function reportToServer(fn, markerData, workspacesRoot) {
...
export async function cleanupAttachments(taskId) {
    try {
...
export async function downloadAttachments(client, token, workspaceId, taskId, attachmentIds) {
...
export async function runSession(input) {
    ...
    const onKill = async () => {
        ...
        if (intent?.reason === "superseded") {
            ...
            catch { /* best-effort — chhlat steering may have already marked it */ }
        }
    ...
    if (input.steeringEnabled && input.steeringMailboxDir && task.contextKey) {
        ...
        // Start watching the mailbox for steered messages from chhlat
        mailboxWatcher = watchInbox(agentBaseDir, task.contextKey, (seq, message) => {
...
async function main() {
    ...
    const client = new ChhlatClient(input.serverURL);
    try {
