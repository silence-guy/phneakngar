 ...
 */
export function canConnectToHeadroom(port) {
    return new Promise((resolve) => {
        ...
        const req = httpRequest({ host: "127.0.0.1", port, path: "/", method: "GET", timeout: PROBE_TIMEOUT_MS }, (res) => {
            const headerNames = Object.keys(res.headers).map((h) => h.toLowerCase());
            const server = String(res.headers["server"] ?? "").toLowerCase();
            const isHeadroom = headerNames.some((h) => h === "x-headroom" || h.startsWith("x-headroom-")) ||
...
export function canRunHeadroom(command, env = process.env, spawn = nodeSpawn) {
    return new Promise((resolve) => {
        let settled = false;
        const finish = (ok) => {
            if (settled)
...
export async function ensureHeadroomProxy(config, paths, deps = {}) {
    if (!config.enabled)
 ...
 */
function writeUpstreamConfig(config, paths) {
