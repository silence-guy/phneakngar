...
export function extractHttpStatus(message) {
    ...
    const semantic = /\b([45]\d{2})\s+(?:Unauthorized|Forbidden|Not Found|Too Many Requests|Internal Server Error|Bad Gateway|Service Unavailable|Gateway Timeout)\b/i.exec(message);
    return semantic ? Number(semantic[1]) : null;
}
export const AUTH_ACTION_REQUIRED_PATTERNS = [
...
function classifyByExplicitToken(message) {
...
function classifyByTextPatterns(message) {
...
export function classifyRuntimeError(message, httpStatus) {
 ...
 */
export function scrubDiagnosticText(text) {
    let scrubbed = text;
    // API tokens
