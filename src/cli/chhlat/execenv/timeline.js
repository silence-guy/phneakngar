...
export function localISOString(now = new Date()) {
...
export function initEntry(timelineDir, entry) {
...
export async function initEntryAsync(timelineDir, entry) {
    const filename = todayFilename();
    const filePath = join(timelineDir, filename);
    const lockPath = lockPathFor(timelineDir, filename);
    try {
...
export function updateEntry(timelineDir, taskId, updater) {
...
export function createTimelineEntry(taskId, prompt, type, sessionId, pid, provider, contextKey, detailedLog) {
    return {
        task_id: taskId,
...
export function findResumableSessionByContextKey(timelineDir, contextKey, provider) {
    const entries = [];
...
export function findRunningPidByTaskId(timelineDir, taskId) {
    for (const filename of recentFilenames(7)) {
 ...
 */
export function steerWarmupGraceMs() {
 ...
 */
export function findSupersedablePredecessor(timelineDir, contextKey, provider, warmupGraceMs, now) {
    for (const filename of recentFilenames(7)) {
...
export { todayFilename as _todayFilename, localISOString as _localISOString, filenameForDate as _filenameForDate, recentFilenames as _recentFilenames, };
