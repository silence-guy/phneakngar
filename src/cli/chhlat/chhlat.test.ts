...
vi.mock("./config.js", () => ({
  ...
  sessionRunnerLogDir: vi.fn(() => path.join("/tmp", "phneakngar", "chhlat", "session-runners")),
  chhlatLogFilePath: vi.fn(() => path.join("/tmp", "phneakngar", "chhlat", "logs", "2026-01-01.log")),
...
vi.mock("./ws-client.js", () => {
  ...
  return { ChhlatWsClient: MockChhlatWsClient };
});
...
vi.mock("./pidfile.js", () => ({
  acquireChhlatPid: vi.fn(() => true),
  releaseChhlatPid: vi.fn(),
}));
...
vi.mock("url", () => ({
  fileURLToPath: vi.fn(() => "/fake/chhlat.ts"),
}));
...
// so chhlat safety-net handlers don't bleed across tests.
...
describe("chhlat session runner dispatch", () => {
  beforeEach(() => {
  ...
  it("activeTasks decrements on child close event", async () => {
    ...
    // Manually invoke a poll by calling the function startChhlat set up
...
describe("chhlat steering with pendingSteer merge", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
...
describe("chhlat with multi-workspace config", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
...
describe("chhlat agent_ids lazy sync", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
...
describe("chhlat startup failures release pidfile", () => {
  beforeEach(async () => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
  ...
  it("registers an exit handler that releases the pidfile", async () => {
    ...
    mockReleaseChhlatPid.mockClear();
    exitHandler!();
    expect(mockReleaseChhlatPid).toHaveBeenCalled();
  });
...
describe("spawnSessionRunner", () => {
  ...
  it("opens a log file fd with taskId.log and passes it as stdio [ignore, fd, fd]", () => {
    ...
    expect(mockMkdirSync).toHaveBeenCalledWith(
      path.join("/tmp", "phneakngar", "chhlat", "session-runners"),
      { recursive: true },
    );
    expect(mockOpenSync).toHaveBeenCalledWith(
      path.join("/tmp", "phneakngar", "chhlat", "session-runners", "t1.log"),
      "a",
    );
  ...
  it("sets logFilePath in the serialized input", () => {
    ...
    expect(decoded.logFilePath).toBe(path.join("/tmp", "phneakngar", "chhlat", "session-runners", "t1.log"));
  });
  ...
  it("deletes oldest files when over limit, keeping newest 500", () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(mockUnlinkSync).toHaveBeenCalledWith(
        path.join("/tmp", "phneakngar", "chhlat", "session-runners", `${i}.log`),
      );
    }
  });
...
describe("chhlat workspace eviction", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
  ...
  it("continues polling remaining workspaces when middle one is evicted (3 workspaces)", async () => {
    ...
    // Chhlat should NOT have shut down (still has ws1 and ws3)
    expect(mockProcessExit).not.toHaveBeenCalled();
  });
  ...
  it("config write failure does not block in-memory eviction", async () => {
    ...
    // saveCLIConfigForProfile threw, but chhlat should NOT crash
...
describe("chhlat 401 handling (no config removal)", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
  ...
  it("does not remove workspace from config on startup 401", async () => {
    ...
    expect(saveCLIConfigForProfile).not.toHaveBeenCalled();
    // Chhlat should still be running (ws1 registered successfully)
    expect(mockProcessExit).not.toHaveBeenCalled();
  });
  ...
  it("does not evict workspace on poll 401, continues polling", async () => {
    ...
    expect(saveCLIConfigForProfile).not.toHaveBeenCalled();
    // Chhlat should NOT shut down — ws1 still active, ws2 just had a transient 401
    expect(mockProcessExit).not.toHaveBeenCalled();
  });
});

describe("chhlat restart via update", () => {
...
describe("chhlat rescan via poll", () => {
  beforeEach(async () => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
  ...
  it("calls requestRestart when pending_rescan is true in poll response", async () => {
    ...
    // requestRestart triggers shutdown + spawn of new chhlat
...
describe("chhlat restart spawn", () => {
  ...
  it("restart spawn uses process.execPath with log file fd for stdio", async () => {
    ...
    expect(mockOpenSync).toHaveBeenCalledWith(
      path.join("/tmp", "phneakngar", "chhlat", "logs", "2026-01-01.log"),
      "a",
      0o600,
    );
...
describe("chhlat kill_task handling", () => {
  beforeEach(() => {
    signalHandlers.clear();
    intervalTimers.length = 0;
    clearedTimers.length = 0;
...
describe("handleWsPush via WebSocket onMessage", () => {
  ...
  async function setupChhlatWithWs() {
    vi.mocked(loadCLIConfigForProfile).mockReturnValue({
      server_url: "",
      watched_workspaces: [{ id: "ws1", name: "Test WS", token: "al_test_token" }],
    });
  ...
  it("handles chhlat.file_requests via WS push with Map lookup", async () => {
    await setupChhlatWithWs();
    expect(capturedWsOnMessage).not.toBeNull();
  ...
  it("handles chhlat.kill via WS push with Map lookup", async () => {
    await setupChhlatWithWs();
    expect(capturedWsOnMessage).not.toBeNull();
  ...
  it("handles chhlat.meetings via WS push with Map lookup", async () => {
    await setupChhlatWithWs();
    expect(capturedWsOnMessage).not.toBeNull();
  ...
  it("skips chhlat.meetings for unknown workspace via WS push", async () => {
    await setupChhlatWithWs();
    expect(capturedWsOnMessage).not.toBeNull();
