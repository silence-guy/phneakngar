...
export type FsAccessPlatform = "darwin" | "linux" | "win32" | "other";

export type ProbeResult = "ok" | "missing" | "blocked";

export interface FsAccessDeps {
  platform?: NodeJS.Platform;
  homedir?: () => string;
  probe?: (dir: string) => ProbeResult | Promise<ProbeResult>;
...
export interface EnsureFsAccessResult extends FsAccessResult {
  opened: boolean;
  throttled: boolean;
}
...
function platformOf(p: NodeJS.Platform): FsAccessPlatform {
  if (p === "darwin" || p === "linux" || p === "win32") return p;
  return "other";
}

function sensitiveDirs(platform: FsAccessPlatform, home: string): string[] {
  switch (platform) {
    case "darwin":
...
function settingsUriFor(platform: FsAccessPlatform): string | undefined {
  if (platform === "darwin") return FDA_URI;
  return undefined;
}

function hintFor(platform: FsAccessPlatform, blocked: string[]): string {
  const list = blocked.join(", ");
  switch (platform) {
    case "darwin":
      return (
        `macOS privacy (TCC) blocks reading ${list}. Grant Full Disk Access to the app that runs ` +
    ...
    case "win32":
      return (
        `Could not read ${list}. On Windows your own files are readable by default, so this usually ` +
    ...
    default:
      return (
        `Could not read ${list}. On Linux your own files are readable by default, so this usually ` +
...
export async function defaultProbe(
  dir: string,
  opts: { timeoutMs?: number; read?: (d: string) => Promise<unknown> } = {},
): Promise<ProbeResult> {
...
export function defaultOpen(target: string): void {
  if (platformOf(process.platform) !== "darwin") return;
  try {
...
export async function checkFilesystemAccess(deps: FsAccessDeps = {}): Promise<FsAccessResult> {
  ...
  const outcomes = await Promise.all(
    dirs.map(async (dir) => ({ dir, r: await probe(dir) })),
  );
...
export function isOpenablePlatform(platform: FsAccessPlatform): boolean {
...
function writeMarker(path: string, ts: number): void {
  try {
...
export async function ensureFilesystemAccess(deps: FsAccessDeps & {
  autoOpen?: boolean;
  ignoreThrottle?: boolean;
  throttleMs?: number;
} = {}): Promise<EnsureFsAccessResult> {
...
export function fsAccessStartupNotes(result: EnsureFsAccessResult): {
  warn?: string;
  info: string[];
} {
  ...
  if (result.opened) {
    ...
    info.push(
      `Settings prompt throttled (once per hour). Run '${cmdPrefix()} grant-access' to open it now.`,
    );
  }
  return {
