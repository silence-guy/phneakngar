...
export async function readDirectoryTree(dirPath, basePath, logicalDirPath = dirPath) {
    let entries;
    let canonicalRoot;
    try {
        canonicalRoot = await realpath(basePath);
        const canonicalDir = await realpath(dirPath);
        if (!isContainedPath(canonicalRoot, canonicalDir))
            return [];
...
function isContainedPath(canonicalRoot, canonicalTarget) {
 ...
 */
export async function validatePath(agentWorkdir, requestedPath) {
