...
**Staging proof artifacts:** [docs/gateway-live-telegram-proof/](./gateway-live-telegram-proof/) — fill `EVIDENCE.template.md` after a real run (never commit bot tokens).
...
9. [ ] **Record evidence** — Copy [EVIDENCE.template.md](./gateway-live-telegram-proof/EVIDENCE.template.md) → `EVIDENCE-YYYY-MM-DD.md` with **no secrets**.
